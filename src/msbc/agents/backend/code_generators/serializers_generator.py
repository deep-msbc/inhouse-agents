"""LLM-based Django serializers.py generator for Stage 3 backend code generation."""
...
class SerializersGenerator:
    async def generate(
        ...
        file_path = Path(scaffold_app_path) / "serializers.py"
        prompt_data = _load_prompt("serializers")

        endpoints_json = json.dumps(endpoints, indent=2)
        models_code_truncated = _truncate(models_code)

        system = _fmt(prompt_data["system"], module_name=app_name)
        base_user = _fmt(
            prompt_data["user_template"],
            module_name=app_name,
            models_code=models_code_truncated,
            endpoints_json=endpoints_json,
            lookup_fields_json=json.dumps(lookup_fields or []),
        )
        ...
        for attempt in range(3):
            ...
            try:
                result, _ = await call_llm_with_schema(
                    ...
                    user_prompt=user,
                    schema=_CODE_SCHEMA,
                    schema_name=f"serializers:{app_name}",
                )
                code: str = result["code"]
            except Exception as exc:
                last_error = str(exc)
                logger.warning("serializers_generator: LLM failed (attempt %d): %s", attempt + 1, exc)
                continue

            is_valid, error = validate_syntax(code)
            if is_valid:
                file_path.write_text(code, encoding="utf-8")
                logger.info("serializers_generator: wrote %s", file_path)
            ...
            last_error = error
            logger.warning(
                "serializers_generator: syntax error '%s' attempt %d: %s",
        ...
        return GeneratedFile(
            ...
            generation_method="llm",
            syntax_valid=False,
            errors=[last_error or "serializers generation failed after 3 attempts"],
        )
